...
nomad_follower:
 - will follow all allocations if it contains service tag ```nomad-follower.enabled=true``` (can be changed with `LOG_TAG` environment variable) on the worker and tail the allocation logs to the aggregate log file. 
